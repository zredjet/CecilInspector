...
`search`は入力ファイルを`--parallel`（既定はCPU数、上限8）の並列度で解析し、結果は入力順に結合するため、出力は逐次処理と同一です。同時に開くファイルの分だけメモリを使うので、大きなDLLが多い環境でメモリを抑えたい場合は`--parallel 1`や`--parallel 2`を指定してください。`dump`はレポートを逐次書き出すため並列化しません。

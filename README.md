...
`search`は入力ファイルを`--parallel`（既定はCPU数で最大8。明示指定に上限はありません）の並列度で解析し、結果は入力順に結合するため、出力は逐次処理と同一です。同時に開くファイルの分だけメモリを使うので、大きなDLLが多い環境でメモリを抑えたい場合は`--parallel 1`や`--parallel 2`を指定してください。`dump`はレポートを逐次書き出すため並列化しません。
...
処理時間の多くはILの読み込みに伴うメモリ割り当てのGCで、既定のワークステーションGCは回収を1スレッドで行うため並列度を上げても伸びが頭打ちになります。多数のファイルを一括で処理する場合は、環境変数`DOTNET_gcServer=1`と`DOTNET_GCHeapCount=4`（ヒープ数の上限。並列度程度が目安）でサーバーGCにすると大きく短縮できますが、メモリ使用量は増えます。目安として、248個・82MBのDLLを16コアのMacで参照検索した場合、逐次で6.9秒（サーバーGCでは5.0秒）、`--parallel 8`で5.3秒（サーバーGCでは3.4秒、RSSは約0.9GBから約1.4GBに増加）でした。上限を設けたい場合は`DOTNET_GCHeapHardLimit=0x40000000`（1GB）のように指定できます。

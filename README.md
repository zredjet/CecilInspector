...
| `--quiet`, `-q` | 標準エラーへの警告・情報の各行を出さない（件数の要約1行と終了コードは変わらない。`CECIL_INSPECTOR_DEBUG=1`のときは無視される。`dump`でも使用可） | 出力する |
| `--color` | `auto`, `always`, `never`。コンソール出力をANSIカラーで色分けする（`dump`でも使用可） | `auto` |
| `--parallel` | 同時に解析するファイル数。`1`で従来どおりの逐次処理（明示指定に上限はない） | CPU数（既定値のみ上限8） |

入力がフォルダの場合は、`.dll`、`.exe`、`.netmodule`を対象にします。単一のmanifest DLLを指定した場合は、そのassemblyに含まれるsecondary netmoduleも検索します。依存DLLは「解析対象DLLの隣接フォルダ → `--reference-path`の指定順 → 入力内のアセンブリ所在フォルダ → フレームワークの既知フォルダ → 実行中ランタイムのアセンブリ → GAC（Windows）」の順で探索します。どの場所でも、ランタイム自身のバインドと同じく、同じ名前・Culture・PublicKeyTokenで同じか新しいVersionの候補を採用します（`netstandard 2.0.0.0`が2.1.0.0のファサードへ解決される、参照より新しいNuGet版がbinにある、など）。参照より古いVersionの候補は採用せず、警告に「候補のパス、Version、要求Version」を含めて理由を示します（Culture／PublicKeyTokenの不一致や読み込めない候補も同様に理由を出します）。フレームワークの既知フォルダとは、実行中のランタイム、`DOTNET_ROOT`や既定のインストール先にある.NETの共有ランタイム（`shared/Microsoft.NETCore.App/<version>`）と参照パック（`packs/Microsoft.NETCore.App.Ref`）、Windowsでは`%WINDIR%\Microsoft.NET\Framework64\v4.0.30319`などの.NET Frameworkフォルダと参照アセンブリです。単一ファイル配布版は自身のランタイムを内蔵しているためディスク上のフォルダが無く、この探索に依存します。たとえばnet8向けDLLは、.NET 8以降のランタイムか参照パックがインストールされていれば解決できます。入力外の依存DLLは`--reference-path`で追加してください。フレームワーク由来の場所から解決したアセンブリは1回の実行で1度だけ開き、すべての入力ファイルで共有します。解決できない依存先があると、ファイルと依存先ごとに1行の警告（未解決メンバー数と例を含む）を標準エラーへ出し、終了コード`3`にします。この警告は`--scope references`/`all`でプロパティ・イベント参照を分類するために依存先を読む必要があるときに出るもので、典型的には入力フォルダに無いNuGet依存や、実行環境にインストールされていないフレームワーク（例: macOS/Linux上で.NET Framework製アプリを解析）が原因です。`CECIL_INSPECTOR_DEBUG=1`で「解決失敗:」の行から依存先を特定し、`--reference-path`で補えます。行数だけを抑えたい場合は`--quiet`を付けると各行の代わりに件数の要約1行だけになります（終了コードは`3`のままです。`CECIL_INSPECTOR_DEBUG=1`を設定した場合は`--quiet`より優先して全診断と解決の追跡を出力します）。特定バージョンの参照アセンブリで解決したい場合は、`--reference-path <dotnet>/packs/Microsoft.NETCore.App.Ref/8.0.x/ref/net8.0` のように参照アセンブリのフォルダを指定してください（隣接フォルダに候補が無ければこのフォルダが先に使われます）。

オプションは位置引数の前後どちらにも置けます。値は`--kind method`と`--kind=method`のどちらの形式でも指定でき、`--reference-path`以外の値付きオプションを2回指定するとエラーになります（`--kind`だけは和集合になります）。`-`で始まる検索文言は`--`の後に指定します（例: `cecil-inspector search app.dll -- -Prefixed --match exact`）。`--`以降は、上表のオプション名そのものを除いて`-`で始まる文言も位置引数として扱うため、`--`の後にオプションを続けて書けます。`--help`／`-h`（または`help`）で使用方法、`--version`／`-v`（または`version`）でバージョンを表示します。どちらも先頭に置けば後続の引数は無視します。

ネイティブDLL、壊れたファイル、アクセス不能なフォルダは標準エラーへ警告を出し、残りを続けます。解析の途中で失敗したファイルについては、そこまでに見つかったヒットを結果に含めたうえで警告を出し、終了コード`3`で結果が不完全であることを示します。フォルダ入力配下で、対象拡張子のファイルと（再帰検索時の）ディレクトリがシンボリックリンク／再解析ポイントの場合は追跡せず、警告と部分成功として扱います（対象外の拡張子のリンクや、`--no-recursive`時のディレクトリのリンクは通常のファイルと同じく無視します）。入力ルート自体がディレクトリのシンボリックリンクの場合は拒否します。ハードリンクはパス単位で解析・集計します。

安全のため、`--output`は既存ファイルを上書きしません。また、レポート出力先に`.dll`、`.exe`、`.netmodule`は指定できません。同じフォルダの`.<名前>.<ランダム>.partial`一時ファイルへ書き、正常完了時だけ指定名へ確定します。Ctrl-C（SIGINT）、SIGTERM、SIGHUP、SIGQUITで中断した場合は、入力フォルダの走査中・解析中・レポート出力中のどの段階でも一時ファイルを削除して終了コード`130`で終了します（2回目の割り込みは即座に強制終了します）。一時パスがシンボリックリンク／再解析ポイントへ差し替えられた場合も確定を拒否します。出力先には、他ユーザーが書き換えられない信頼できるフォルダを指定してください。

### 並列処理と性能

`search`は入力ファイルを`--parallel`（既定はCPU数で最大8。明示指定に上限はありません）の並列度で解析し、結果は入力順に結合するため、出力は逐次処理と同一です。並列時は大きいファイルから順に着手し、最後に大きなDLLが残って全体を引き延ばさないようにします。同時に開くファイルの分だけメモリを使うので、大きなDLLが多い環境でメモリを抑えたい場合は`--parallel 1`や`--parallel 2`を指定してください。`dump`はレポートを逐次書き出すため並列化しません。

参照検索（`--scope references`／`all`）はメソッド本体のIL全体を走査するため、定義検索よりはるかに重い処理です。必要な種別だけを`--kind`で絞ると、たとえば`--kind method,property`では型参照の展開を省けます。

処理時間の多くはILの読み込みに伴うメモリ割り当てのGCです。ワークステーションGCは回収を1スレッドで行い、その間すべての解析スレッドが止まるため、数百ファイルでは並列度を上げても逐次と変わらない時間になります。そのため既定でサーバーGC（ヒープ数4、`System.GC.ConserveMemory=9`）を有効にしています。目安として、700個・305MBのDLLを16コアのMacで参照検索した場合、旧来のワークステーションGCでは`--parallel 8`でも15秒だったものが、既定設定では約4秒（RSSは約1.9GBから約2.1GB）です。環境変数はこの既定より優先されるので、`DOTNET_gcServer=0`で従来のワークステーションGCに戻せ、`DOTNET_GCHeapCount`でヒープ数を変えられ（並列度程度が目安。増やすと速くなる代わりにメモリが増えます）、上限を設けたい場合は`DOTNET_GCHeapHardLimit=0x40000000`（1GB）のように指定できます。
...
**Visual Studioの外部ツール**: `--format msbuild`を付けると、ヒットを1行ずつ`パス(行,列): info CI0001: [definition/method] シンボル`の形式で出力します（この形式では検索文言を含む`Query:`行を出力しません。`Kinds:`／`Assemblies:`／`Matches:`／`Breakdown:`のヘッダー行と件数の省略行は出力しますが、`path(line,col):`で始まらないため問題マッチャーには掛かりません）。**ツール > 外部ツール**に次のように登録し、「出力ウィンドウを使用」（Use Output window）を有効にすると、出力ウィンドウの行をダブルクリック、または「次のメッセージへ移動」で該当行にジャンプできます。
...
`$(SolutionDir)`は末尾に`\`を付けて展開されるため、`"$(SolutionDir)"`と書くと`\"`がエスケープされた引用符として解釈され、引数が壊れます。末尾に`.`を付けた`"$(SolutionDir)."`で回避してください。
...
検索レポートとダンプはコンソールと一時ファイルへ逐次書き込むため、レポート全文をメモリへ蓄積しません。完了後に一時ファイルを`--output`の指定名へ確定します。解析中のファイル差し替えによる部分ヒット混入を防ぐため、各対象ファイルは解析前に丸ごとメモリへ読み込み、そのコピーに対して必要な部分だけを遅延解析します。依存アセンブリは読み取り共有でファイルを開いたまま参照します（メモリへ複製すると隣接フォルダ同士を参照するDLL群の走査で使用メモリが倍増するため）。依存アセンブリの探索順は対象ファイルのフォルダで決まるため、同じフォルダの対象ファイルは1つのリゾルバと開いた依存アセンブリ、およびメンバー解決の結果を共有し、そのフォルダの最後のファイルを解析し終えた時点で閉じます（`CECIL_INSPECTOR_DEBUG=1`の解決トレースはフォルダごとに1回出ます）。走査中に依存DLLを差し替えると、そのファイルの分類が警告になることがあります。同じファイルに束縛される複数のVersion要求は1度だけ開いて共有します。
